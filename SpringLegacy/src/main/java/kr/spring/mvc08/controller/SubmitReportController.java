...
public class SubmitReportController {
	...
	public String submit(@Valid @ModelAttribute("report") SubmitReportVO vo, BindingResult result) throws IllegalStateException, IOException { // 초기화할 때 ModelAttribute와 같은 이름 명시, 이순서대로 명시해야 함
		...
		// 지정한 경ㄹ에 파일 저장
		vo.getReportFile().transferTo(file); // throws Exception 이나 try~catch 문 사용해야 하는데 에러 메세지를 보기 위해 throws Exception 사용

		System.out.println("주제 : " + vo.getSubject());
		System.out.println("업로드한 파일 : " + vo.getReportFile().getOriginalFilename());
		System.out.println("파일 용량 : " + vo.getReportFile().getSize() + " bytes");
