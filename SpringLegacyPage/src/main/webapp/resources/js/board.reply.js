$(function(){
	...
	function selectList(pageNum){
		...
		$.ajax({
			...
			success:function(param){
				...
				기존의 문자열은 '또는 "로 감싸고 + 기호로 연결해야 변수 사용 가능하지만
				템플릿 리터럴은 백틱(`,어금문자)를 이용해 문자열을 보다 간결하게 작성.
				문자열 안에서 변수 삽입 (Express Interpolation) : ${변수명} 사용해 변수나 표현식을 문자열에 삽입
				*/
				...
				$(param.list).each(function(index,item){
					...
					if(item.click_num == 0 || param.user_num != item.click_num){
						output += `
								<img class="output-rfav" src="../resources/images/heart01.png" data-num="${item.re_num}">
								<span class="output-rfcount">${fav_cnt}</span>
						`;
					}else{
						output += `
								<img class="output-rfav" src="../resources/images/heart02.png" data-num="${item.re_num}">
								<span class="output-rfcount">${fav_cnt}</span>
						`;
					}
	...
	$(document).on('click','.modify-btn',function(){
		...
		// 이전에 이미 수정하는 댓글이 있을 경우 수정 버튼을 클릭하면 숨김 sub-item를 환원시키고 수정 폼을 초기화함
		initModifyForm();
		...
		$.ajax({
			...
			success:function(param){
				if(param.result=='logout'){
					...
					$('#mre_form').parent().find('p').html(useBrNoHtml($('#mre_content').val()));
	...
	$(document).on('click', '.delete-btn',function(){
		...
		$.ajax({
			url:'deleteReply.do',
			type:'post',
			data:{re_num:re_num,mem_num:mem_num}, // :기준 앞은 키 뒤는 변수(value)
			dataType:'json',
			success:function(param){
				if(param.result == 'logout'){
	...
	$('#re_form').submit(function(event){
		...
		let formData = $(this).serialize(); // form이하 데이터를 전부 읽어옴
	...
	$(document).on('click','.output-rfav',function(){
		// 이벤트가 발생한 태그 정보 저장
		const heart = $(this); // success부분에서 호출하면 이벤트 발생한게 사라져 다르게 인식하기 때문에 미리 상태를 받아둬야함
		$.ajax({
			url:'writeReFav.do',
			type:'post',
	...
	/*------------------------------
	 좋아요 표시
	------------------------------- */
	function displayFav(param,heart){ // 매개변수 heart는 등록/삭제할 때 사용
		let output;
		...
		heart.parent().find('.output-rfcount').text(param.count);
	}
	...
	$(document).on('keyup','textarea',function(){
		...
		if(inputLength>300){ // 300자를 넘어선 경우
			$(this).val($(this).val().substring(0,300)); // 잘라내서 300자에 맞춤			
		}else{ // 300자 이하인 경우
			// 남은 글자수 구하기
			let remain = 300 - inputLength;
			remain += '/300';
