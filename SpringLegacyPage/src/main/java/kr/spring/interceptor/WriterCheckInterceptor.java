...
public class WriterCheckInterceptor extends HandlerInterceptorAdapter{
	private static final Logger log = LoggerFactory.getLogger(WriterCheckInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
		...
		log.debug("<<로그인한 회원번호>> : " + user.getMem_num());
		...
		if(user.getMem_num()!=board.getMem_num()) {
			...
			request.setAttribute("accessMsg", "로그인 회원과 작성자 회원 불일치");
			request.setAttribute("accessUrl", request.getContextPath()+"/board/list.do");
			
			// 일부로 forward 방식 사용 
			RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/common/accessDenied.jsp");
